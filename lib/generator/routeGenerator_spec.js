...
describe('RouteGenerator functionalities', function () {
  ...
  it('should use the webpack require when specified in the config file', function () {
    ...
    let componentFile = fse.readFileSync(configCustom.getRoutesRoot() + '/' + routeName.toLowerCase() + path.sep + `${routeName.toLowerCase()}.component.ts`, 'utf8');
...
function chekIfFilesExistsAndAreCorrects(pathToCheck) {
  let componentFile = fse.readFileSync(pathToCheck + path.sep + `${routeName.toLowerCase()}.component.ts`, 'utf8');
  ...
  let controllerFile = fse.readFileSync(pathToCheck + path.sep + `${routeName.toLowerCase()}.controller.ts`, 'utf8');
  expect(controllerFile, '[controllerFile]').to.have.string(`export class ${capitalizedRouteName}Controller`);

  let moduleFile = fse.readFileSync(pathToCheck + path.sep + `${routeName.toLowerCase()}.module.ts`, 'utf8');
  ...
  let routeFile = fse.readFileSync(pathToCheck + path.sep + `${routeName.toLowerCase()}.route.ts`, 'utf8');
  ...
  let htmlFile = fse.readFileSync(pathToCheck + path.sep + `${routeName.toLowerCase()}.template.html`, 'utf8');
  expect(htmlFile, '[htmlFile]').to.have.string(`Hello from ${capitalizedRouteName} !`);
}

function checkIfRootIndexIsCorrectlyCreated() {
  let indexFile = fse.readFileSync(path.join(config.getRoutesRoot(), 'index.ts'), 'utf8');
