...
describe('ConfigResolver functionalities', function () {
  ...
  it('should get the default value for the update or create flag', function () {
    ...
    expect(cg.getUpdateOrCreate()).to.be.true;
  });
  ...
  it("should get all its data from the custom conf file", function () {
    let customJsonData = {  "componentsRoot": "src/cmp", "routesRoot": "src/rts",
      "componentsRootModuleName": "cmp.ts", "routesRootModuleName": "rts.ts", "updateOrCreate": false };
    ...
    expect(configResolver.getUpdateOrCreate()).to.be.false;
